...
#pragma region VulkanGraphicsBackendApi
	...
	void VulkanGraphicsBackendApi::EndCommandBuffer(const CommandBufferAllocInfo& Info)
	{
		vkEndCommandBuffer(*_CommandBuffers.Get(Info.CommandBufferHandle));
		_ActiveCommandBufferHandle = 0;
		_ActiveCommandBuffer = nullptr;
	}

	void VulkanGraphicsBackendApi::RenderBegin(const CommandBufferAllocInfo& Info, uint32_t FrameIndex)
	{
		auto ActiveCommandBuffer = *_CommandBuffers.Get(Info.CommandBufferHandle);
		vkWaitForFences(_Data.Device.GetHandle(), 1, &_Data.InFlightFence, VK_TRUE, UINT64_MAX);

		VkResult result = vkAcquireNextImageKHR(_Data.Device.GetHandle(), _Data.SwapChainKHR.GetHandle(), UINT64_MAX,
			_Data.ImageAvailableSemaphore, VK_NULL_HANDLE,
		...
		vkResetFences(_Data.Device.GetHandle(), 1, &_Data.InFlightFence);
	...
	void VulkanGraphicsBackendApi::RenderEnd(const CommandBufferAllocInfo& Info)
	{
		auto commandBuffer = _ActiveCommandBuffer;
		EndCommandBuffer(Info);

		{
			...
			if (vkQueueSubmit(_Data.Device.GetQueue(QueueFamilies::GRAPHICS), 1, &submitInfo, _Data.InFlightFence) != VK_SUCCESS)
	...
	void VulkanGraphicsBackendApi::BindGraphicsPipeline(size_t PipelineHandle)
	{
		vkCmdBindPipeline(_ActiveCommandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, _GraphicsPipelineSet.Get(PipelineHandle)->GetHandle());
	}

	void VulkanGraphicsBackendApi::BindVertexBuffers(size_t* BufferHandles, uint32_t Count)
	{
		// 🆕 Bind vertex buffer
	...
	void VulkanGraphicsBackendApi::BindIndexBuffer(size_t IBHandle)
	{
	...
	size_t VulkanGraphicsBackendApi::AllocateBuffer(const BufferCreateInfo& Info)
	{
		if (Info.SizeInBytes == 0 || Info.Data == nullptr)
		...
		return _BufferSet.Create(VulkanBuffer{ Buffer, Allocation, AllocationInfo });
	}

	void VulkanGraphicsBackendApi::MapBufferData(size_t BufferHandle, void* Data, size_t Size, uint32_t Offset)
	{
	...
	void VulkanGraphicsBackendApi::FreeBuffer(size_t BufferHandle)
	{
		auto Buffer = _BufferSet.Get(BufferHandle);
	...
	size_t VulkanGraphicsBackendApi::CreateGraphicsPipeline(const GraphicsPipelineCreateInfo& CreateInfo)
	{
		VulkanGraphicsPipeline Shader;
		Shader.Init(CreateInfo, _Data.Device.GetHandle(), _Data.SwapChainKHR.GetFormat());
		return _GraphicsPipelineSet.Create(Shader);
	}

	void VulkanGraphicsBackendApi::DestroyGraphicsPipeline(size_t PipelineHandle)
	{
	...
	void VulkanGraphicsBackendApi::_CreateSynchornization()
	{
		...
		fenceInfo.flags = VK_FENCE_CREATE_SIGNALED_BIT; // 👈 very important

		auto device = _Data.Device.GetHandle();
		if (vkCreateSemaphore(device, &semaphoreInfo, nullptr, &_Data.ImageAvailableSemaphore) != VK_SUCCESS ||
			vkCreateSemaphore(device, &semaphoreInfo, nullptr, &_Data.RenderFinishedSemaphore) != VK_SUCCESS ||
			vkCreateFence(device, &fenceInfo, nullptr, &_Data.InFlightFence) != VK_SUCCESS) {
	...
	{
		auto device = _Data.Device.GetHandle();
		vkDestroySemaphore(device, _Data.ImageAvailableSemaphore, nullptr);
		vkDestroySemaphore(device, _Data.RenderFinishedSemaphore, nullptr);
		vkDestroyFence(device, _Data.InFlightFence, nullptr);
	}
#pragma endregion
